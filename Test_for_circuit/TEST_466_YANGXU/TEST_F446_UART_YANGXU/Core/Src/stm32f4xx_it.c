...
extern uint8_t ReceiveData5[];
extern uint8_t ReceiveData4[];
	...
	if(UART4 == huart4.Instance)
		{
			if(RESET!=__HAL_UART_GET_FLAG(&huart4,UART_FLAG_IDLE))
			{
				...
				HAL_UART_Transmit_DMA(&huart4,ReceiveData4,Rx_len);
	  ...
	  if(RESET!=__HAL_UART_GET_FLAG(&huart5,UART_FLAG_IDLE))
		{
			...
			HAL_UART_Transmit_DMA(&huart5,ReceiveData5,Rx_len);
