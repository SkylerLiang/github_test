...
void Encoder_Init(void)
{
	...
	Pos_Init[2] = Encoder2_Read(&hspi2);
	tmp_last[2] = Pos_Init[2];
	
	
	SPI_Set_Speed(&hspi3, PCLK_32);
	for(uint8_t i=0; i<50; i++)//滤波
	{
		//Encoder1_Read(&hspi1);
		Encoder3_Read(&hspi3);
	...
	Pos_Init[3] = Encoder3_Read(&hspi3);
...
uint16_t Encoder3_Read(SPI_HandleTypeDef* hspi)
{
...
void Encoder_Cal(void)
{
	...
	tmp_last[1] = tmp[1];*/
	tmp[2] = Encoder2_Read(&hspi2);
	tmp[3] = Encoder3_Read(&hspi3);
