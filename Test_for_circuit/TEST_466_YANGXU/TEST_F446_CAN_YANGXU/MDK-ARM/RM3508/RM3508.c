...
M3508_PID M3508_Pos_Pid[8] = //云台0.2, 0.009 0.01  底盘0.2 0 0
{
	...
	{.Kp = 5,.Ki = 0.005,.Kd = 0,.Max = 14000,.Min = -14000,.IntegralLimit = 20000,.DeadBand = 0},	//ID = 4
...
=======
	 	RM3508_CAN_Send_Data(&hcan2, RM3508_Sendbuf1, send_id ,8);
>>>>>>> a7dc6b2230f0a18941949e553be245e90850b693
...
void RM3508_Set_Speed(int goal_speed,int ID)
{
	...
	M3508_Speed_Pid[id].Err_last = M3508_Speed_Pid[id].Err;//上一次的error
	M3508_Speed_Pid[id].Err 	= M3508_Speed_Pid[id].Target -  M3508_Speed_Pid[id].NowIS;//这一次的error
  ...
  M3508_Speed_Pid[id].Err_sum += M3508_Speed_Pid[id].Err; 
	...
	M3508_Speed_Pid[id].PID_Out = CLAMP(M3508_Speed_Pid[id].PID_Out,M3508_Speed_Pid[id].Min,M3508_Speed_Pid[id].Max);//限制，在一个最大值和最小值之间
...
void RM3508_Get_Feedback(uint32_t std_id,uint8_t* data_p)
{
	...
	for(i=1;i<9;i++)
	{
		if(std_id==0x200+i)
		{
			memcpy(RM3508_Feedback_Buf [i-1],data_p,7);//data_p中有8个元素，取前7个字节，因为第八个字节是NULL值，没有用
...
{
	int torque = 0;
	if(RM3508_Feedback_Buf[motor_id-1][2]>>7==1)//最高位是不是1，如果是1就证明是负数
...
void RM3508_Pos_Rec(uint8_t motor_id)//记录位置
{
	int id=motor_id-1;
