...
M3508_PID M3508_Speed_Pid[8] =//8 0 0    8 0 0   
{
	...
	{.Kp = 8,.Ki = 0,.Kd = 0,.Max = 16000,.Min = -16000,.IntegralLimit = 1000,.DeadBand = 10},	//ID = 3
...
uint8_t RM3508_Set_I(int target_i,uint8_t motor_id)
{
    ...
    if(send_id==0x200)//ID1-4由can1发
		{    RM3508_FDCAN_Send_Data(&hfdcan1, RM3508_Sendbuf2, 0x200,8);
				 RM3508_FDCAN_Send_Data(&hfdcan2, RM3508_Sendbuf2, 0x200,8);
				 RM3508_FDCAN_Send_Data(&hfdcan3, RM3508_Sendbuf2, 0x200,8);
	  }
//    else
//		 RM3508_FDCAN_Send_Data(&hfdcan2, RM3508_Sendbuf1, send_id ,8);
...
/********************************************************************************
  ...
  *@  output    : 成功返回0，失败返回2
*********************************************************************************/
uint8_t RM3508_FDCAN_Send_Data(FDCAN_HandleTypeDef* hfdcan, uint8_t *pData, uint16_t ID, uint16_t Len)
{
//	HAL_StatusTypeDef HAL_RetVal = HAL_ERROR;
//	uint8_t FreeTxNum = 0;
//	FDCAN_TxHeaderTypeDef TxHeade;
//	
//	TxHeade.IDE = CAN_ID_STD;  //标准帧,CAN_ID_EXT扩展帧;
//	TxHeade.RTR = CAN_RTR_DATA;  //数据帧,CAN_RTR_REMOTE遥控帧
//	TxHeade.StdId = ID;
//	TxHeade.DLC = Len;
//	
...
//	
//	HAL_RetVal = HAL_FDCAN_AddTxMessage(hfdcan,&TxHeade,pData,(uint32_t*)FDCAN_TX_MAILBOX0);
//	
...
//	
//	return 0;
	FDCAN_SendData(&hfdcan1,pData,ID,Len);
	FDCAN_SendData(&hfdcan2,pData,ID,Len);
	FDCAN_SendData(&hfdcan3,pData,ID,Len);
	return 0;
}
/********************************************************************************
